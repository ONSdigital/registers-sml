...
object AssertionHelpers {
  ...
  def assertDataFrameStringEquality(expected: RawDataTableList, bounds: String)(castExpectedMandatoryFields: (DataFrame, String) => DataFrame): DataFrame = {
    val expectedOutputDF = nullToNull(createDataFrame(expected))
    val castedExpectedOutputDF = castExpectedMandatoryFields(expectedOutputDF.withColumn(colName = cellNumber, expectedOutputDF.col(cellNumber).cast(IntegerType)), bounds)
