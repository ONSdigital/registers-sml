...
trait Imputor extends Serializable{
  ...
  def imputeEmployees(turnover:Option[String], payeEmployees:Option[String], tph:Option[String]):(String,String) = {
    ...
    try{
      inputs match{
        case (Some(trn),None,Some(tph)) => (null,(trn.toInt / tph.toInt).toString) //turnover != null, paye employees = null
        case (None,Some(emps),Some(tph)) => ((emps.toInt * tph.toInt).toString,null)//paye employees!= null, turnover  = null
        case (Some(trn),Some(emps),Some(tph)) => ((emps.toInt * tph.toInt).toString,(trn.toInt / tph.toInt).toString)//turnover != null, paye employees != null
